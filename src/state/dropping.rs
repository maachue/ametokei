use super::{Cell, CellType, Column, EachFrameImpl, Mode, ShouldRender, buffer::RenderBuffer};
...
impl DroppingState {
    fn gen_drop(&self, rb: &mut RenderBuffer, seed: u64) {
        ...
        for g in 0..groups {
            let range = if groups.saturating_sub(1) == g {
                last_group
            } else {
                GROUP_SIZE
            };
    ...
    fn new_drop(&self, rb: &mut RenderBuffer, seed: u64) {
        ...
        rb.line.iter().enumerate().for_each(|(i, d)| {
            if let Some(cell) = rb
                .buf
                .get_mut(i)
                .unwrap()
...
impl EachFrameImpl for DroppingState {
    fn on_frame(
        &mut self,
        rb: &mut super::buffer::RenderBuffer,
        seed: u64,
        frame: u64,
    ) -> ShouldRender {
        // each column
